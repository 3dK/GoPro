...
#		Evaluate VP9, AV1, ... codecs for storage and eventual streaming (browser support)
#
...
# Video enconding settings (no hardware support)
# x264: superfast 26
# x265: ultrafast 28
# VP9: ?
set FFMPEG_ENCODE_SETTINGS "-c:a copy -c:v libx264 -preset veryfast -crf 29"
	...
	"convert" {
		...
		if { [llength $videoFiles] } {
			...
			foreach videoFile $videoFiles {
				...
				set pid [eval exec $NICE $IONICE $FFMPEG -y -i $videoFile $FFMPEG_ENCODE_SETTINGS $outputFile >>& [regsub {\.mp4$} $outputFile {.log}] &]
